...
# replace them
purrr::map2(
  .x = xct_xref_citations_match$key_missing,
  .y = xct_xref_citations_match$key_match,
  ~ ngr::ngr_str_replace_in_files(text_current = .x, text_replace = .y, files = file_list)
)

# now we can check for missing keys with more likelyhood there will not be incorrect matches.
keys_missing <- xciter::xct_bib_keys_missing(
  path_bib = path_bib,
  citations = rbbt::bbt_detect_citations(
    list.files(
...
# we check the matches to be 100% sure they are air tight (hopefully)
