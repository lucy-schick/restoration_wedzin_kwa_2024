...
gis_params_raw_to_update <- gis_params_raw_all|> 
  ...
  #                 !str_detect(source_layer, "bcfishobs|bcfishpass")) |> 
    dplyr::filter(!is.na(source_layer) & 
                  ...
                  !str_detect(source_layer, "bcfishobs|bcfishpass|chinook|skeena_east")) |> 
  # we know that tenures and the land ownership layers are problematic so lets remove
  dplyr::filter(!str_detect(source_layer, "whse_forest_tenure.ften_range_poly_carto_vw|whse_cadastre.pmbc_parcel_fabric_poly_svw"))
...
gis_params_raw_all_updated <- dplyr::bind_rows(
  gis_params_raw_all %>% 
    dplyr::filter(is.na(source_layer) | 
                    is.na(source_column_name) |
                    stringr::str_detect(source_layer, "bcfishobs|bcfishpass") |
  ...
  dplyr::mutate(is_wet_cultural_site = column_name == "wet_cultural_site") %>%
  dplyr::arrange(desc(is_wet_cultural_site), rank, source_layer, source_column_name, column_name) %>%
