...
gis_params_raw_all_updated <- dplyr::bind_rows(
  gis_params_raw_all %>% 
    dplyr::filter(is.na(source_schema_table) | 
                   ...
                   stringr::str_detect(source_schema_table, "whse_forest_tenure.ften_range_poly_carto_vw")),
...
read_csv("data/restoration_site_priority_parameters.csv") |> 
  ...
  dplyr::arrange(column_name_raw) |> 
  dplyr::select(column_name_raw, source_column_name, everything()) |> 
  fpr::fpr_kable(caption_text = "Proposed parameters used to document and rank future potential restoration/enhancement sites.",
                 scroll = gitbook_on)
