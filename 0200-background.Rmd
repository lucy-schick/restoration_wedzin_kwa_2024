...
dfo_sad_tr <- readr::read_csv('data/inputs_raw/All Areas NuSEDS.csv') %>% 
  ...
  dplyr::select(species, analysis_yr, 
...
```{r fig-dfo-sad-tr1, fig.cap = "Fisheries and Oceans Canada stock assessment data for the Bulkley River - Upper.  Data is filtered to only include records with total return to river data."}

ggplot(dfo_sad_tr, aes(x = analysis_yr, y = total_return_to_river, group = species)) +
  geom_line() +
...
```{r fig-dfo-sad-nad1, fig.cap = "Fisheries and Oceans Canada stock assessment data for the Bulkley River - Upper.  Data is filtered to only include records with natural adult spawners data."}

dfo_sad_raw %>% 
  dplyr::filter(waterbody == "BULKLEY RIVER - UPPER") %>% 
  dplyr::filter(!is.na(natural_adult_spawners)) %>% 
  arrange(species, analysis_yr) %>% 
  ggplot(aes(x = analysis_yr, y = natural_adult_spawners, group = species)) +
  geom_line() +
  facet_wrap(~species, scales = "free") +
  labs(x = "Year", y = "Natural Adult Spawners") +
  ggdark::dark_theme_bw()
```
...
```{r fig-dfo-sad-tr2, fig.cap = "Fisheries and Oceans Canada stock assessment data for the Morice River.  Data is filtered to only include records with total return to river data."}

dfo_sad_raw %>%
  ldfo_sad_plot_line("MORICE RIVER", "total_return_to_river", "species", "analysis_yr", col_group_exclude = "Coho")

```

```{r fig-dfo-sad-nad2, fig.cap = "Fisheries and Oceans Canada stock assessment data for the Morice River.  Data is filtered to only include records with natural adult spawners data."}
